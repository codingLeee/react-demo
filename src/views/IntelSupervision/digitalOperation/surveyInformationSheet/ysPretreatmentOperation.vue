<!-- 排水户管道预处理作业前现场记录表 - 雨水 -->
<template>
    <div class="ys_pretreatment_operation">
    ...
    <el-table max-height="500px" :data="tableData" class="digitalManagement_table">
      ...
      <el-table-column prop="waterLevel" label="雨水井水位(cm)">
        <template slot-scope="scope">
  ...
  methods: {
    ...
    async updateTable({ row, $index }) {
      ...
      if (_this.currUpdateIndex == $index) {
        ...
        if (row["isAdd"]) {
          ...
          // 修改- 保存
          await updateScene({ ...row, pipeType: 1 }).then(res => {
            _this.isHasAdd = false;
            _this.currUpdateIndex = -1;
          });
        }
    ...
    updateForm() {
      ...
      if (_this.isUpdateForm) {
        updateSceneHeader({ ..._this.baseInfoForm, pipeType: 1 })
          .then(res => {
            _this.$emit("getTableData");
          })
          .catch(err => {
